...
function showForm(templateId) {
  document.querySelectorAll('.labels__form').forEach(form => {
    // Ховаємо з анімацією
    form.classList.remove('isShow');

    if (!form.classList.contains('isShow')) {
    form.style.display = 'none';
    }

  });

  const formToShow = document.querySelector(`.labels__form.${templateId}`);
  if (formToShow) {
    formToShow.style.display = 'grid';
    setTimeout(() => {
      formToShow.classList.add('isShow');
    }, 10); // даємо час браузеру оновити стиль
  }
}
